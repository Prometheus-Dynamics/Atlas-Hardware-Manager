...
pub(crate) fn is_supported_flash_target_path(path: &str) -> bool {
...
pub(crate) fn flash_with_rust_stream(
    image_path: &Path,
    device_path: &str,
...
#[cfg(target_os = "linux")]
pub(crate) fn collect_mounted_partitions_from_lsblk_entry(
    entry: &Value,
...
#[cfg(target_os = "linux")]
pub(crate) fn mounted_partitions_for_target_linux(
    device_path: &str,
...
#[cfg(target_os = "linux")]
pub(crate) fn ensure_target_partitions_unmounted_linux(device_path: &str) -> Result<(), String> {
...
pub(crate) fn open_image_reader_for_flash(image_path: &Path) -> Result<Box<dyn Read>, String> {
    let input = fs::File::open(image_path)
...
pub(crate) fn emit_flash_verify_progress(
...
pub(crate) fn discard_reader_bytes(reader: &mut dyn Read, mut bytes_to_skip: u64) -> Result<(), String> {
    if bytes_to_skip == 0 {
        return Ok(());
    }
...
pub(crate) fn verify_streams_match(
    source: &mut dyn Read,
    target: &mut dyn Read,
    bytes_total: Option<u64>,
    progress_context: Option<&FlashProgressContext>,
    offset_base: u64,
    ignored_mismatch_ranges: Option<&[(u64, u64)]>,
) -> Result<u64, String> {
    fn is_ignored_mismatch_offset(offset: u64, ranges: &[(u64, u64)]) -> bool {
        ranges.iter().any(|(start, length)| {
    ...
    loop {
        ...
        if source_buffer[..source_read] != target_buffer[..source_read] {
            let first_non_ignored_mismatch = source_buffer[..source_read]
                .iter()
                ...
                .find_map(|(index, (left, right))| {
                    ...
                    let offset = offset_base + bytes_verified + index as u64;
                    if ignored_mismatch_ranges
                        .map(|ranges| is_ignored_mismatch_offset(offset, ranges))
                        .unwrap_or(false)
                    {
                        None
                    } else {
            ...
            if let Some(mismatch_offset) = first_non_ignored_mismatch {
                return Err(format!(
...
#[derive(Clone, Copy)]
pub(crate) struct PartitionSpan {
    pub(crate) part_type: u8,
    pub(crate) start_lba: u64,
    pub(crate) sectors: u64,
}

pub(crate) fn read_mbr_partition_spans(image_path: &Path) -> Result<Option<Vec<PartitionSpan>>, String> {
...
#[cfg(target_os = "linux")]
pub(crate) fn spawn_linux_verify_reader(
    device_path: &str,
...
pub(crate) fn verify_flashed_target_range(
    image_path: &Path,
    device_path: &str,
