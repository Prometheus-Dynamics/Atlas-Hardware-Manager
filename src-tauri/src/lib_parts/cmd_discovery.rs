...
fn discovered_device_matches_identity(
...
fn compact_chip_value(value: &str) -> String {
    let compacted = value
        .split_whitespace()
        .collect::<Vec<_>>()
        .join(" ")
...
fn fallback_network_device_display_name(runtime_product: Option<&str>) -> String {
...
fn is_likely_roborio_ip_candidate(candidate: &IpDeviceCandidate) -> bool {
    if candidate
        .mac
        .as_deref()
        .map(is_probable_roborio_mac)
...
fn has_network_candidate_signal(candidate: &IpDeviceCandidate) -> bool {
